...
function CardDatabaseTab({ session }: { session: NonNullable<import('../lib/auth').Session> }) {
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      // Upload image to Supabase Storage instead of storing as base64
      let imageUrl = '';
      const imageData = dbCroppedImage || dbImage;
      if (imageData) {
        // Convert base64 to blob
        const res = await fetch(imageData);
        const blob = await res.blob();
        const ext = blob.type === 'image/png' ? 'png' : 'jpg';
        const fileName = `${Date.now()}-${cardName.trim().replace(/\s+/g, '-').toLowerCase()}.${ext}`;
        const { data: uploadData, error: uploadError } = await sb.storage
          .from('card-images')
          .upload(fileName, blob, { contentType: blob.type, upsert: true });
        if (uploadError) throw uploadError;
        const { data: urlData } = sb.storage.from('card-images').getPublicUrl(uploadData.path);
