...
exports.handler = async (event) => {
  ...
  // Anything below that throws (a missing global like `fetch` on an old Node
  // runtime, a network hiccup calling Supabase, etc.) would otherwise escape
  // as an unhandled exception - which Netlify reports to the browser as a
  // bare, detail-free 502. Catching it here means a real error message
  // reaches the teacher instead of just "Failed".
  try {
    return await handle(event, headers);
  } catch (err) {
    console.error('reset-pin crashed:', err);
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Server error: ' + (err?.message || String(err)) }) };
  }
};
