...
exports.handler = async (event) => {
  ...
  // See the equivalent comment in reset-pin.js: without this, an unhandled
  // exception here would just surface to the browser as a bare 502.
  try {
    return await handle(event, headers);
  } catch (err) {
    console.error('delete-student crashed:', err);
